...
def get_cookie():
    print('Enter usernames separated by spaces: ', end='', flush=True)
    input_line = sys.stdin.readline().rstrip()
    uniqnames = sorted(input_line.split())
    print(f'Using cookie for [{",".join(uniqnames)}]')

    raw = hashlib.sha256(('|'.join(uniqnames).lower()).encode()).digest()
    cookie = int.from_bytes(raw, byteorder='little') % 0x7FFF

    with open('cookie', 'wb') as out:
        out.write(bytes(input_line + '\n', 'ascii'))
